...
void Compiler::createListingHeader()
{
	...
	listingFile << "STAGE0:  Anh Minh Le      " << ctime(&now) << endl;
...
string Compiler::whichValue(string name)		// tells which value a name has
{
	...
	else	//name is an identifier and hopefully a constant
	{
		...
		if (finding != symbolTable.end() && finding->second.getValue() != "")
			...
			processError("reference to undefined constant456");
	}
...
char Compiler::nextChar()		// returns the next character or END_OF_FILE marker
{
	...
	if (sourceFile.eof())		// if it is the end of file function
		ch = END_OF_FILE;
	else if ((ch == '\n' || countLine == 1) && endF == false)		// if ch is go to another line
	{
		listingFile << endl << right << setw(5) << countLine << '|';
		lineNo = countLine;		// get the current line
...
void Compiler::assignStmt() {//production 4
	if (isNonKeyId(token)) {
		...
		nextToken();
		if (token != ":=") {
			processError("expected \":=\" after non-key-ID");
		}
		pushOperator(token);
		nextToken();
		express();
	...
	else { processError("non-keyword id expected\n"); }
}
...
void Compiler::writeStmt() {
	if (token == "write") {
		...
		if (token == "(") {
			...
			x = ids();
			if (token != ")") {
				processError(" ) is needed for readList");
			}
		}
		code("write", x);
...
void Compiler::term() {
		...
		isInteger(token) || isNonKeyId(token))
	{
		...
		processError("Invalid expression: not, true, false, (, +, -, non-key ID, or integer expected in factor");
	}
}
...
void Compiler::factor() {
	if (token == "not" || isBoolean(token) || token == "(" || token == "+" || token == "-" ||
		...
		processError("Invalid expression: not, true, false, (, +, -, non-key ID, or integer expected in part");
	}
}
...
//a = (b);
void Compiler::part() {
	//5 case //after check, move nextToken
	//case 1: not 
	if (token == "not") {
		nextToken();
		//now check what after not - ex- not true, not varA, not (express)
		if (token == "(") {
			express();
			if (token != ")") {
		...
		if (token == "(") {
			...
			processError(" expected a '(', integer, or non-key ID");
		}
	}
	else if (token == "-") {
		nextToken();
		...
		processError("Illegal Token in Part()");
	}
	nextToken();
...
void Compiler::emitSubtractionCode(string operand1, string operand2) {
	//NOTE: Op1 : rhs, Op2:lhs
//PSU: if both of them are not integer -> error 
	if (operand2 != "") {
		if (!isInteger(operand1) || !isInteger(operand2)) {
			processError("Illegal type of operand. Can not do subtraction");
		}
		//PSU:if A Register holds a temp not operand1 nor operand2 then //(a + a) + T0
		...
		//PSU:if neither operand is in A register then
		if (contentsOfAReg != operand1 && contentsOfAReg != operand2) {
			//emit code to load operand2 into A register
...
void Compiler::emitDivisionCode(string operand1, string operand2) {
	if (!isInteger(operand1) || !isInteger(operand2)) {
		processError("Illegal type of operand. Can not do division");
	}
...
void Compiler::emitAssignCode(string operand1, string operand2)
{
	if (whichType(operand1) != whichType(operand2)) {
		processError("Incompatible type in Assign Code");
	}
...
void Compiler::emitAndCode(string operand1, string operand2) // op2 && op1			
{
	if (whichType(operand1) != BOOLEAN && whichType(operand2) != BOOLEAN) {
		processError("Illegal Type when compare");
	}
...
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
void Compiler::emitOrCode(string operand1, string operand2) // op2 || op1			//Not DOne
{
	if (whichType(operand1) != BOOLEAN && whichType(operand2) != BOOLEAN) {
		processError("Illegal Type when compare");
	}
...
void Compiler::emitEqualityCode(string operand1, string operand2) {
	...
	emit("", "je", labelN, operand2 + " = " + operand1 + " then jump to set eax to TRUE");
...
void Compiler::emitInequalityCode(string operand1, string operand2) {
	...
	string labelN = getLabel(); //L0
	//je                 .L4          ; h = f then jump to set eax to TRUE
	emit("", "jne", labelN, operand2 + " <> " + operand1 + " then jump to set eax to TRUE");
	//  mov     eax,[FALSE]             ; else set eax to FALSE
	emit("", "mov", "eax,[FALSE]", "; else set eax to FALSE");
	symbolTable.insert({ "false", SymbolTableEntry("FALSE", BOOLEAN, CONSTANT, "0", YES, 1) });
	string labelN_plus_one = getLabel();
...
void Compiler::emitReadCode(string operand, string) {
	...
	for (auto x : operand) {
		if (x != ',') {
			...
			listOperand.push_back(currentToken);
			currentToken = "";
		}
	}
	listOperand.push_back(currentToken);
	...
	for (auto name : listOperand) {
		...
		if (whichType(name) == INTEGER)
		{
			...
			if (definedStorage == false) {
				...
				emit("FALSLIT", "db", "\'TRUE\' , 0", "; literal string FALSE");
